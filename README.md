...
`verification/verify_with_gemini.py` drives the skill through a real Gemini (Google AI Studio free tier) call: the model receives `SKILL.md`'s body as its system prompt and exactly one generic tool — `run_shell(command)`, restricted server-side to invocations starting with `wikitrends` — so the model has to read the instructions and compose the actual command itself, the same way a host agent (e.g. Claude Code) would use this skill in practice. Run it with `GOOGLE_API_KEY=... python verification/verify_with_gemini.py`; it writes transcripts and the resulting PDFs/PNGs under `verification/output_*/`. `verification/check_numeric_consistency.py` then checks that every number in the model's final answer matches a field in that call's `--json` output.

**Status:** the harness is implemented and the skill itself has been smoke-tested end-to-end via `verification/run_examples.sh` (real Wikimedia/Wikidata API calls, real 1-page PDFs, `pypdf`-checked) for all three reference query shapes — including confirming live that the flagship "intermittent fasting" comparison has no Polish Wikipedia article, and that an ambiguous topic ("learning English") correctly hard-fails with candidates instead of silently mis-resolving. The live-Gemini run itself requires a `GOOGLE_API_KEY`, which is supplied at run time and never committed; if this repo doesn't yet contain a `verification/output_*/` directory, that step is still pending a key.
